function [results,chain,s2chain] = dynamicabcdram(model,data,params,options,method)
...
for isimu=2:nSimu
    ...
    % multivariate Gaussian unbiased
    ...
    % multivariate Gaussian shifted half a standard deviation
    if strcmp(method,'shift')
        
        % gets set to an index from 1 to 20
        highest_misfit_data = whichdataisworst(oldpar,data);
        
        change = worstdatadirection(highest_misfit_data,oldpar,data);
        
        update = randn(1,nPar)*R;
        update = update+(change*0.5*diag(qcov)');
    ...
    if strcmp(method,'skew')
        ...
        update = zeros(1,nPar);
        for ii = 1:nPar
            update(ii) = rskewnorm(0,qcov(ii,ii),change);
        end
    ...
    if strcmp(method,'trunc')
        ...
        if change > 0
            for ii = 1:nPar
                update(1,ii) = trandn(-0.5,inf)*qcov(ii,ii);
            end
        end
        
        if change < 0
            for ii = 1:nPar
                update(1,ii) = trandn(-inf,0.5)*qcov(ii,ii);
            end
        end
