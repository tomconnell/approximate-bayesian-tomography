function [results,chain,s2chain] = blockeddramrun(model,data,params,options)
...
%     update = randn(1,nPar)*R;
%     
%     which_par = rem(isimu,2)+1;
%     
%     newpar = oldpar;
%     
%     if which_par == 1
%         newpar(1) = newpar(1)+update(1);
%         newpar(2) = newpar(2)+update(2);
%         newpar(5) = newpar(5)+update(5);
%     elseif which_par == 2
%         newpar(3) = newpar(3)+update(3);
%         newpar(4) = newpar(4)+update(4);
%         newpar(5) = newpar(5)+update(5);        
%     end
%     
%     accept = 0;
    ...
    if any(newpar<bounds(1,:)) || any(newpar>bounds(2,:))
        ...
        newss  = ssfun(newpar,data); % sum-of-squares
