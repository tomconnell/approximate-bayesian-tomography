function [results,chain,s2chain] = gravblockeddramrun(model,data,params,options)
...
oldss = ssfun(oldpar,data,ones(1,10)*1000000,0);
...
for isimu=2:nSimu
    ...
    %% Change 1 par at a time
    
    update = randn(1,nPar)*R;
    
    % gets set to an index from 1 to 10
    highest_misfit_data = whichdataisworst(oldpar,data);
    
    which_par = selectpartochange(highest_misfit_data);
    
    tolerance = settolerance(which_par);
    % temper tolerance scale for acceptance rate?
    ...
    if any(newpar<bounds(1,:)) || any(newpar>bounds(2,:))
        ...
        newss  = ssfun(newpar,data,tolerance,1); % sum-of-squares
        oldss = ssfun(oldpar,data,tolerance,0);
        %oldss = ssfun(oldpar,data,0);
        %newss = ssfun(newpar,data,1);
